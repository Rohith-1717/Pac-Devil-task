...
class Game extends Phaser.Scene {
  ...
  preload(){
    ...
    this.load.spritesheet("coinSpin", "assets/coin_sheet.png", {
      frameWidth: 384,
      frameHeight: 1024
    });
  }
  ...
  positions.forEach(p=>{
    const c = this.coins.create(p.x, p.y, "coinSpin");
    c.setScale(0.1);
    c.play("coin-spin");});
    ...
    this.scoreText = this.add.text(650, 550, "Score: 0",{
      color: "#fff",
      fontSize: "20px"});

    this.best = parseInt(localStorage.getItem("bestScore") || "0");
    this.bestText = this.add.text(650, 575, `Best: ${this.best}`, {
    color: "#ff0",
    fontSize: "20px"});
